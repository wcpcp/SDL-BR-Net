...
class Tester(Trainer):
    def __init__(self, model1, model2, loss, CFG, checkpoint, test_loader, dataset_path, show=False):
        ...
        # self.conv = nn.DataParallel(nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0).cuda())
        
        self.dataset_path = dataset_path
        self.show = show
        self.model1.load_state_dict(checkpoint['state_dict1'])
        self.model2.load_state_dict(checkpoint['state_dict2'])
        # self.conv.load_state_dict(checkpoint['conv_state_dict'])
        # self.conv.load_state_dict(torch.load('/root/FR-UNet-master/saved/two_stage/240406112523/checkpoint-epoch2.pth')['conv_state_dict'])
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (img, gt, dgt) in enumerate(tbar):
                ...
                # pre1, f3, f4, f5 = self.model1(img)
                # pre = self.model2(f3, f4, f5)
                
                # pre1 = self.model1(img)
                # pre1 = nn.Sigmoid()(pre1)
                # new_input = self.conv(torch.cat((pre1, img), dim=1))
                # pre = self.model2(pre1)
