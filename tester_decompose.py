...
class Tester(Trainer):
    ...
    def test(self):
        ...
        tic = time.time()
        with torch.no_grad():
            for i, (img, gt, dgt, cgt) in enumerate(tbar):
                self.data_time.update(time.time() - tic)
                img = img.cuda(non_blocking=True)
                gt = gt.cuda(non_blocking=True)
                dgt = dgt.cuda(non_blocking=True)
                cgt = cgt.cuda(non_blocking=True)
                ...
                if self.show:
                    ...
                    img = np.transpose(img[0].cpu().numpy(), (1, 2, 0))
                    img = np.uint8(img * 255)
                    cv2.imwrite(f"save_picture/img{i}.png", img)
                    # img.save(f"save_picture/img{i}.png")
                    # cv2.imwrite(
