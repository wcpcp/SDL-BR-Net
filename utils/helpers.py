...
'''
getattr 是 Python 的内置函数，用于获取对象的属性或方法。
getattr(module, config[name]['type']): 从module中获取名为config[name]['type']的属性或方法。
'''
def get_instance(module, name, config, *args):
    return getattr(module, config[name]['type'])(*args, **config[name]['args'])

def get_optimizer_instance(module, name, config, *args):
    return getattr(module, config[name]['type'])(*args, lr=1e-4,weight_decay=1e-5)
