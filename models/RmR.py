...
class rr(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.down_over_pixel = DSConv3x3(in_c, in_c, dilation=2)
        ...
        self.brrelu = nn.Sequential(
            nn.Conv2d(2*in_c, out_c, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(out_c),
            nn.ReLU(inplace=False)
        )
...
#         x_down_0 = self.down_over_pixel(x_down_0)
#         x_down_1 = self.down_over_pixel(x_down_1)

#         x_res0 = self.up0(x_down_0)
#         x_res1 = self.up1(x_down_1)

#         x_out = self.brrelu(torch.cat([x_res0, x_res1], dim=1))
...
class RMR(nn.Module):
    def __init__(self, in_c=32, out_c=32):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3*out_c, out_c, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(out_c),
            nn.LeakyReLU(0.1, inplace=True)
        )
    ...
    def forward(self, F3=None, F4=None, F5=None):
        F3_ = self.rr3(F3)
        F4_ = self.rr4(F4)
        F5_ = self.rr5(F5)

        F = self.conv(torch.cat([F3_, F4_, F5_], dim=1))
        f1 = self.dw1(F)
        f2 = self.dw2(F)
        f3 = self.dw3(F)
        out = self.pre(torch.cat([f1, f2, f3, F], dim=1))

        # out = fusion(F3_, F4_, F5_)
